...
metrics_file = Path(target_folder) / "metrics.json"
...
def summarize_context():
    exclude_keys = ["status", "dataset", "JEN", "JEX", "ontology", "api_spec", "scenario"]

    metrics = load_metrics(metrics_file)
    context_keys = [key for key in metrics.keys() if not any(exclude in key for exclude in exclude_keys)]
    ...
    new_entry = {
        ...
        "performance": {
            "time": {
                "latency": 0,  # Example latency in seconds
                "tpot": 0,  # Example time to process
                "ttft": None  # Example time to first token, if applicable
            },
            "tokens": {
                "input_tokens": 0,  # Example input tokens count
                "cache_creation_input_tokens": 0,  # Example cache creation input tokens
                "cache_read_input_tokens": 0,  # Example cache read input tokens
                "output_tokens": 0  # Example output tokens count
            }
        },
        "requirements": ""
    }

    # Add Metrics of context keys to the new entry
    for key in context_keys:

        if "step_name" in metrics[key].keys():

            new_entry["prompt"] += metrics[key]["prompt"]
            new_entry["thinking"] += metrics[key]["thinking"]
            new_entry["response"] += metrics[key]["response"]

            new_entry["performance"]["time"]["latency"] += metrics[key]["performance"]["time"].get("latency", 0)
            # new_entry["performance"]["time"]["tpot"] += metrics[key]["performance"]["time"].get("tpot", 0)
            # new_entry["performance"]["time"]["ttft"] = metrics[key]["performance"]["time"].get("ttft", None)

            new_entry["performance"]["tokens"]["input_tokens"] += metrics[key]["performance"]["tokens"].get("input_tokens", 0)
            new_entry["performance"]["tokens"]["cache_creation_input_tokens"] += metrics[key]["performance"]["tokens"].get("cache_creation_input_tokens", 0)
            new_entry["performance"]["tokens"]["cache_read_input_tokens"] += metrics[key]["performance"]["tokens"].get("cache_read_input_tokens", 0)
            new_entry["performance"]["tokens"]["output_tokens"] += metrics[key]["performance"]["tokens"].get("output_tokens", 0)
            
            # new_entry["requirements"] += metrics[key].get("requirements", "")

        elif "tool_name" in metrics[key].keys():
            # Tool use detected

            new_entry["prompt"] += str(metrics[key]["tool_input"])
            new_entry["thinking"] += str(metrics[key]["tool_result"])
            new_entry["response"] += str(metrics[key]["tool_result"])

    new_metrics = {}

    for key in metrics.keys():
        if key != "get_endpoint_from_api_spec" and (key not in context_keys and "scenario" not in key):
            new_metrics[key] = metrics[key]
        
    new_metrics["summarize_context"] = new_entry

    for key in metrics.keys():
        if key not in context_keys and "scenario" in key:
            new_metrics[key] = metrics[key]

    # Save the new metrics to a file
    output_path = eval_folder / "summarized_metrics.json"
    with open(output_path, 'w') as file:
        json.dump(new_metrics, file, indent=4)
    print(f"Summarized metrics saved to {output_path}")
...
def extract_reasoning_sentences(text):
    ...
    cognitive_patterns = [
        ...
        r'\bprecise\b',
        r'\bsemantic\b',
        r'\bsupplementary node\b',
        r'\bspatial connection\b'
    ]
    pattern = re.compile('|'.join(cognitive_patterns), re.IGNORECASE)
...
def is_factual_or_structural(sentence):
    exclusion_patterns = [
    r'\bhas (property|location|value|relationship)\b',
    r'\bis (a|an|the|connected|positioned|located|mapped|integrated|included|situated)\b',
    r'\bcontains\b',
    r'\bfollows\b',
    r'\bmaps to\b',
    r'\bcorresponds to\b',
    ...
    return any(re.search(p, sentence, re.IGNORECASE) for p in exclusion_patterns)
